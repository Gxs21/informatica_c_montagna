...
/* FUNZIONE DA REALIZZARE
   ...
   La funzione deve modificare L rimuovendone l'elemento centrale (se lunghezza dispari) o gli elementi
   centrali (se lunghezza pari).

   Ad esempio, se L = (10, 99, -3, 12, 79, 28), al termine
   dell'esecuzione si deve avere L = (10, 99, 79, 28) (viene
   rimossa la coppia di elementi centrali di valore -3 e 13, rispettivamente).
   Nel caso di lista con numero dispari di elementi, L = (10, 99, -3),
   l'output dev'essere L = (10, -3).
   ...
   Se lo si ritiene utile, e' possibile definire ulteriori funzioni di supporto.
...
void rimuovi_centrali(struct list *L) {
    ...
    if(size < 3) return;
    int center = size / 2 - (size % 2 == 0 ? 1 : 0);
    for(int i = 0; i < center - 1; i++) L = L->next;
    struct list *next = size % 2 == 0 ? L->next->next->next : L->next->next;
    if(size % 2 == 0){
        free(L->next->next);
    }
    free(L->next);
